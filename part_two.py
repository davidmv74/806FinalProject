...
class DAN(nn.Module):
    ...
    def __init__(self):
        ...
        hidden_dim = 299      # Also 200 hidden states, we could change this as hyper parameter (they use around 250 on paper)
...
def run_adversarial_epoch(label_train_data, domain_train_data, label_model, domain_model, domain_optimizer, label_optimizer, size):
    ...
    for i in tqdm(range(numBatches)):
        ...
        totalLoss = label_loss-(10**-6)*domain_loss
        totalLoss.backward()
...
def testing_android_temp(model, test_data):
    ...
    for dev_sample in test_data:
        ...
        for i in range(len(candidates)):
            candidateEncoding = model.forward(Variable(torch.FloatTensor(candidates[i])))
            candidatesCosine.append((target[i], cos(refQ, candidateEncoding).data[0]))
        sortedCosines = sorted(candidatesCosine, key = lambda x: x[1], reverse=True)
        sortedRanks = [cand[0]  for cand in sortedCosines]
        allRanks.append(sortedRanks)
    evaluation = Evaluation(allRanks)
    print("MAP", evaluation.MAP())
    print("MRR", evaluation.MRR())
    print("P@1", evaluation.Precision(1))

def unsupervised_test(test_data):
    cos = nn.CosineSimilarity(dim=0, eps=1e-6)
    allRanks = []
    for dev_sample in test_data:
        target = [x[1] for x in dev_sample[1:]]
        embeddings = [x[0] for x in dev_sample]
        refQ = torch.FloatTensor(embeddings[0])
        candidates = embeddings[1:]
        candidatesCosine = []
        for i in range(len(candidates)):
            candidateEncoding = torch.FloatTensor(candidates[i])
            candidatesCosine.append((target[i], cos(refQ, candidateEncoding).numpy()))
        sortedCosines = sorted(candidatesCosine, key = lambda x: x[1], reverse=True)
        sortedRanks = [cand[0]  for cand in sortedCosines]
        allRanks.append(sortedRanks)
    evaluation = Evaluation(allRanks)
    ...
    for dev_sample in test_data:
        ...
        for i in range(len(candidates)):
            candidateEncoding = torch.FloatTensor(candidates[i])
            candidatesCosine.append(cos(refQ, candidateEncoding).numpy()[0])
        auc.add(np.array(candidatesCosine), np.array(target))
    print(auc.value(0.05))
